...
const createToken = async (user) => { // Create a tokens & data from user
  ...
  const access_token = jwt.sign({ id, email, groups }, getSecret('sign', 'access'), options)
...
const login = async (req, res) => {
  try {
    ...
    //Change for Totoz usecase
    const user = await findUser({
     'identities.email': req.body[AUTH_USER_FIELD_LOGIN]
    })
    ...
    const twoFA = await find2FA({member_id: user.id, type: 'TwoFactor::App'})
    const password = req.body[AUTH_USER_FIELD_PASSWORD]
    if (!bcrypt.compareSync(password, user['password_digest']) || !password) return res.status(401).json({ message: 'Incorrect credentials' })
    

    //Get User Details
    const id = user[AUTH_USER_FIELD_ID_FOR_JWT]
    //if (!id) return res.status(401).json({ message: 'Authorization Format Error' })
    if (!USE_OTP && twoFA.activated) {
      // Currently supports only Google Authenticator
      // Fido2 can be added in future
      return res.status(200).json({otp: true, id: user.member_id})
    }
    const tokens = await createToken(user) // 5 minute expire for login
    setTokensToHeader(res, tokens)
    return res.status(200).json(tokens)
